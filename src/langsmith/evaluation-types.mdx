...
Run offline evaluations client-side using the LangSmith SDK (@[Python][langsmith-python] or @[TypeScript][langsmith-js]) or server-side via the [Prompt Playground](/langsmith/prompt-engineering-concepts#prompt-playground) or by [binding evaluators to a dataset](/langsmith/bind-evaluator-to-dataset).
