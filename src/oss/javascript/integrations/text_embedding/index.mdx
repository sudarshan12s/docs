...
<Accordion title="Oracle AI Vector Search">

Install dependencies:

<CodeGroup>
```bash npm
npm i @langchain/oracle oracledb @langchain/core
```
```bash yarn
...
const embeddings = new OracleEmbeddings(connection, {
  provider: "database",
  model: process.env.DEMO_ONNX_MODEL ?? "VECTOR_EMBEDDING_MODEL",
});
```
...
<Columns cols={3}>
  ...
  <Card
    ...
    icon="link"
    href="/oss/integrations/text_embedding/oracleai"
    arrow="true"
    cta="View guide"
  />
  <Card
