...
<Tip>
Learn how to [deploy your deep agents to LangSmith](/oss/langchain/deploy) for production-ready hosting with built-in observability, authentication, and scaling.
</Tip>

<Accordion title="Deep agent `useStream` parameters">

In addition to the [standard `useStream` parameters](/oss/langchain/streaming/frontend), deep agent streaming supports:
...
In addition to the [standard return values](/oss/langchain/streaming/frontend), deep agent streaming provides:
...
- [Streaming overview](/oss/deepagents/streaming/overview) — Server-side streaming with deep agents
- [Subagents](/oss/deepagents/subagents) — Configure and use subagents with deep agents
- [LangChain frontend streaming](/oss/langchain/streaming/frontend) — General `useStream` documentation
...
- [Agent Chat UI](/oss/langchain/ui) — Prebuilt chat interface for LangGraph agents
